...
[2025-11-14] HubSpot/Deals: excluimos empresas con `name` vacío del conteo de "Companies". La API calcula `companiesUniqueCount` filtrando por `name` en la unión (deal + contactos) y la UI lo utiliza si está disponible. Links: `src/app/api/hubspot/deals/route.ts`, `src/components/forms/deal-selector.tsx`, `memory-bank/03-active-decisions.md`
[2025-11-14] UI: añadido botón “El contacto no viene de ManyChat” en el aviso amarillo y en el modal. Permite continuar sin `mp_live_chat_url`; si hay empresa pendiente se asocia al contacto directamente. Links: `src/components/forms/deal-selector.tsx`
[2025-11-14] UI: bypass oculta el aviso y habilita el flujo. Al clickear “El contacto no viene de ManyChat” se oculta el banner y, si luego se crea una empresa, la asociación al contacto salta la validación de ManyChat. Links: `src/components/forms/deal-selector.tsx`
[2025-11-14] HubSpot/Deals: al crear una empresa, el `dealname` se actualiza automáticamente al nombre de la empresa. Se agregó `PATCH /api/hubspot/deals/update` y se integró en `DealSelector`. Links: `src/app/api/hubspot/deals/update/route.ts`, `src/components/forms/deal-selector.tsx`, `memory-bank/03-active-decisions.md`